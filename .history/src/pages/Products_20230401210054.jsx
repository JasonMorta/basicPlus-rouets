...
//scroll to selected name on homepage
  useEffect(() => {
    let poductTitles = []
    console.log('cardRef.current', cardRef.current.childNodes)
    
    if (cardRef.current !== null) {
      cardRef.current.childNodes.forEach((item, i) =>{
        let itemName = item.querySelector(`#cardImage${i}`).textContent
          ...
          {location.pathname.includes(`${itemState?.id}`) ? (
            ...
            products.map((item, i) => (
              <Card style={{ width: "18rem" }} key={i}>
                <Card.Img id={`cardImage${i}`} variant="top" src={item.images[0]} />
                <Card.Body>
